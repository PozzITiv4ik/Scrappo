...
const { player, playerConfig, playerState, playerPos, config, state, utils } = internal;
const { clamp } = utils;
...
const addPlayerExperience = (amount) => {
  ...
  while (remaining > 0) {
    ...
    if (remaining >= needed) {
      ...
      remaining -= needed;
      playerState.xp = 0;
      playerState.xpToNext = player.getXpForLevel(playerState.level);
...
const resetPlayerState = () => {
  ...
  } else if (typeof player.resetState === "function") {
    player.resetState();
  }
...
const applyPlayerDamage = (amount, now) => {
  ...
  const soundApi = registry.get("sound");
  if (soundApi && typeof soundApi.playPlayerHit === "function") {
    soundApi.playPlayerHit();
  }
};
