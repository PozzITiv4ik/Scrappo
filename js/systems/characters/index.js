...
const DEFAULT_SKIN = "assets/icons/main_character.png";
...
const getCharacterSkin = (character) => character?.skin || character?.icon || DEFAULT_SKIN;

const getWeaponSprite = (character) => {
  const weaponId = character?.weaponId || DEFAULT_WEAPON_ID;
  return WEAPONS[weaponId]?.sprite || WEAPONS[DEFAULT_WEAPON_ID]?.sprite || "assets/weapons/pistol/weapon_pistol.png";
};
...
const applyCharacter = (character) => {
  ...
  combatModifiers = getCombatTotals(character);
  selectedCharacterId = character.id;

  const playerApi = registry.get("player");
...
registry.set("character", characterSystem);
const SCRAPPO = window.SCRAPPO || (window.SCRAPPO = {});
SCRAPPO.characterSystem = characterSystem;
window.SCRAPPO_CHARACTER_SYSTEM = characterSystem;
