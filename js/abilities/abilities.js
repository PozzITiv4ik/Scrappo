...
(() => {
  const translations = window.SCRAPPO_ABILITY_TEXTS || {};
  window.SCRAPPO_ABILITIES = window.SCRAPPO_ABILITIES.map((ability) => {
    const text = translations[ability.id];
    if (!text) {
      return ability;
    }
